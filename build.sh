...
build_container() {
    ...
    # Build
    cat $NAME/build.sh | sed -e "s|\%DUDE\%|${DUDE}|" -e "s|\%BUILD_DIR\%|${BUILD_DIR}|" -e "s|\%IP_C\%|${IP_C}|" | ssh -t -i ssh-key/openxt -oStrictHostKeyChecking=no build@192.168.${IP_C}.1${NUMBER}
}
